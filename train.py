...
def get_checkpoint(log_dir, index_to_load):
    ...
    for f in file:
        ...
        if index_to_load in num:
            version = os.listdir(f"{f}/lightning_logs")[0]
            check_name = os.listdir(f"{f}/lightning_logs/{version}/checkpoints/")[0]
            # checkpoint_name = f.replace("[","\[").replace("]","\]").replace("\'","\\'")+"/lightning_logs/"+ version+"/checkpoints/"+check_name
            checkpoint_name = f"{f}/lightning_logs/{version}/checkpoints/{check_name}"
...
def train(hparams, *args):
    ...
    if hparams.multi:
        ...
        test_model_seq2seq(
            hparams, model.model, model.tokenizer, dev_val_loader, time="FINAL"
        )
        ...
        for task_num, (task_id, task_loader) in enumerate(train_loader.items()):
            ...
            if (hparams.CL == "LAMOL") and (
                current_task_to_load is None
                or task_num >= current_task_to_load
            ):
            ...
            if hparams.CL not in ["LAMOL", "EWC"]:
